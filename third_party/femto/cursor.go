...
// WordStartRight moves the cursor forward to the beginning of the next word,
// landing on its first character. It skips the remainder of the current word
// and then any following separators (whitespace, punctuation, line breaks).
func (c *Cursor) WordStartRight() {
	// Skip to the end of the word the cursor is currently within.
	for IsWordChar(string(c.RuneUnder(c.X))) {
		if c.Loc == c.buf.End() {
			return
		}
	...
	// Skip separators until the first character of the next word.
	for !IsWordChar(string(c.RuneUnder(c.X))) {
		if c.Loc == c.buf.End() {
			return
		}
